...
CHOICES=(
	"Romeokub        Update Romeokub itself and run any migrations"
...
if [[ "$CHOICE" == "<< Back"* ]] || [[ -z "$CHOICE" ]]; then
	...
	case "$INSTALLER" in
	"Romeokub") INSTALLER_FILE="$Romeokub_PATH/bin/Romeokub-sub/migrate.sh" ;;
	"ollama") INSTALLER_FILE="$Romeokub_PATH/install/terminal/optional/app-ollama.sh" ;;
	*) INSTALLER_FILE="$Romeokub_PATH/install/terminal/app-$INSTALLER.sh" ;;
	esac
